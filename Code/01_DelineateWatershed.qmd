...
shpFiles <- here("Results/ShapeFiles/") #where to save final shape files, prob need to figure out how to make 1 geopackage for all this
scratchDir <- paste0(tempdir(), "/")#path to temp dir
dir.create(paste0(scratchDir, "basins"), showWarnings = F) #temp dir for intermediate DEMs, if it doesn't exist make dir
...
writeRaster(knzDem, paste0(scratchDir, "1mMosaic.tif"), overwrite = T)
...
st_write(pourPoints, paste0(scratchDir,"pourPoints.shp"), delete_dsn = T)
```
...
wbt_unnest_basins(
  d8_pntr = "flowdir.tif",
  pour_pts = "snap.shp", 
  output = paste0(scratchDir, "basins/sheds.tif") ,
  wd = scratchDir,
)

#Get a list of the watersheds created by `unnest_basins`
sheds <- list.files(paste0(scratchDir, "basins/"), full.names=TRUE)
...
#Load stream vector
streamNet <- st_read(file.path(scratchDir, "streams.shp"))
...
# Add slope and contributing area to the stream network
slope_raster <- rast(file.path(scratchDir, "slope.tif"))
slope_raster <- crop(slope_raster, sheds)
flow_acc_raster <- rast(file.path(scratchDir, "flowacc.tif"))
flow_acc_raster <- crop(flow_acc_raster, sheds)
strahler_raster <- rast(file.path(scratchDir, "strahler.tif"))
...
kingsSites <- pointshp2segvert(path = scratchDir, layer = "snap", rivers = kingsNetwork) 
...
pourPoints <- st_read(paste0(scratchDir, "snap.shp"))
